...
public interface AstSupportMutationInterceptor extends MutationInterceptor {
  ...
  default Collection<MutationDetails> intercept(Collection<MutationDetails> mutations, Mutater mutater) {
    ...
    Function<MutationDetails, Optional<Statement>> nodeForMutation = mutation -> source.getAst(mutation)
        .flatMap(type -> type.findFirst(Statement.class, node -> matchesMutation.test(node, mutation)));
